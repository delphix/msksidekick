...
class masking():
    def __init__(self, config, **kwargs):
        ...
        self.systemalgorithmlist = ['AccNoLookup','AccountTK','AddrLine2Lookup','AddrLookup','BusinessLegalEntityLookup','CommentLookup','CreditCard','DateShiftDiscrete','DateShiftFixed','DateShiftVariable','dlpx-core:CM Alpha-Numeric','dlpx-core:CM Digits','dlpx-core:CM Numeric','DrivingLicenseNoLookup','DummyHospitalNameLookup','EmailLookup','FirstNameLookup','FullNMLookup','LastCommaFirstLookup','LastNameLookup','NameTK','NullValueLookup','RandomValueLookup','RepeatFirstDigit','SchoolNameLookup','SecureShuffle','SM-UNI:NUMERIC','SM-UNI:STRING','SM-US_LATAM','SsnTK','TelephoneNoLookup','USCitiesLookup','USCountiesLookup','USstatecodesLookup','USstatesLookup','WebURLsLookup','SSN SM','ACCOUNT', 'ACCOUNT_TK','ADDRESS', 'ADDRESS', 'BUSINESS', 'COMMENT', 'CREDIT', 'DATE', 'DATE', 'DATE', 'DR', 'DUMMY_HOSPITAL_NAME_SL', 'EMAIL', 'FIRST', 'FULL_NM_SL', 'LAST_COMMA_FIRST_SL', 'LAST', 'NAME_TK', 'NULL', 'PHONE', 'RANDOM_VALUE_SL', 'SCHOOL', 'SECURE', 'SSN_TK', 'USCITIES_SL', 'US_COUNTIES_SL', 'USSTATE_CODES_SL', 'USSTATES_SL', 'WEB_URLS_SL', 'ZIP+4']
                        ...
                        if chk_status == 0:
                            ...
                            if job_exec_response is not None:
                                if job_exec_response['status'] == 'RUNNING':
                                    ...
                                    print_red_on_white = lambda x: cprint(x, 'red', 'on_white')
                                    print_red_on_white(
                                        " Execution status of Masking job# {} on Engine {} : {}. Queued job will start as soon as slots are free.".format(jobid, engine_name, curr_job_status))
                            else:
                                print_red_on_white = lambda x: cprint(x, 'red', 'on_white')
                                print_red_on_white(
